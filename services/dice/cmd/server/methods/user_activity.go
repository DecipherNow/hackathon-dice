...
func (s *serverData) UserActivity(ctx context.Context, request *pb.UserRequest) (*pb.UserActivityResponse, error) {
	...
	for dbRows.Next() {
		dbRows.Scan(&created_at)
	}
	dbRows.Close()
	// And see if its been over 10 minutes
	then, err := time.Parse(time.RFC3339, created_at)
	if err != nil {
		db.Close()
		return nil, err
	}
	...
	if dataisold {
		logger.Debug().Float64("duration", duration.Minutes()).Str("created_at", created_at).Msg("dataisold")
		...
		_, err = stmtCache.Exec(created_at)
		if err != nil {
			db.Close()
			return nil, err
		}
	...
	reponame := ""
	for rowsEvents.Next() {
		err = rowsEvents.Scan(&created_at, &eventtype, &reponame, &summaryline)
		if err != nil {
			db.Close()
			return nil, err
		}
